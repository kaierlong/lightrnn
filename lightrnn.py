	...
	def __init__(self, opt, reuse=None):
		...
		with tf.variable_scope("model", reuse=reuse), tf.name_scope("{}_model".format(opt.mode)):
			...
			with tf.name_scope("embedding"):
				...
				embedding_r = tf.get_variable("embedding_r", [opt.lightrnn_size, opt.embedding_size], initializer=tf.random_uniform_initializer(-stdv, stdv))
				embedding_c = tf.get_variable("embedding_c", [opt.lightrnn_size, opt.embedding_size], initializer=tf.random_uniform_initializer(-stdv, stdv))

			input_r = tf.nn.embedding_lookup(embedding_r, self.data_r)
			input_c = tf.nn.embedding_lookup(embedding_c, self.data_c)
			...
			with tf.variable_scope("RNN"):
				for time_step in range(opt.num_steps):
					...
					if opt.mode == "train":
						...
						"""
						true_input_data_rc = self.target_r[time_step]
						inputs_rc = tf.nn.embedding_lookup(embedding_r, true_input_data_rc)
						...
						inputs_rc = tf.nn.embedding_lookup(embedding_r, all_outputs_r)
			...
			with tf.name_scope("loss"):
				...
				loss_r_summary = tf.summary.scalar("loss_r", loss_r)
				loss_c_summary = tf.summary.scalar("loss_c", loss_c)
				loss_summary = tf.summary.scalar("loss", self.loss)
				
				self.loss_summary_op = tf.summary.merge([loss_summary, loss_r_summary, loss_c_summary])
